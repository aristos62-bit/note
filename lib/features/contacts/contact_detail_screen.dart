...
class _ContactDetailScreenState
  ...
  Widget build(BuildContext context) {
    DebugConfig.provider(
        'ContactDetailScreen build id=${widget.itemId}');
  ...
  void _syncPropsFromDB(List<ItemProperty> props) {
    ...
    final email   = props.where((p) => p.key == 'email')
        .firstOrNull?.value ?? '';
    final company = props.where((p) => p.key == 'company')
        .firstOrNull?.value ?? '';
    final website = props.where((p) => p.key == 'website')
        .firstOrNull?.value ?? '';
    final address = props.where((p) => p.key == 'address')
        .firstOrNull?.value ?? '';
    final notes   = props.where((p) => p.key == 'notes')
        .firstOrNull?.value ?? '';
    final bdStr   = props.where((p) => p.key == 'birthday')
        .firstOrNull?.value;
    ...
    if (_notesCtrl.text.isEmpty && notes.isNotEmpty)
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Center(
              child: Text(letter,
                  style: context.h1.withColor(color)),
            ),
          ),
