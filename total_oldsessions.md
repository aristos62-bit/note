...
## Session 40 — 12-06-2026 (Attachment Field + Auto-Lock Overhaul + Edge Cases)

### Στόχος
Προσθήκη `FieldType.attachment` στις Collections (upload file ανά entry) + διόρθωση auto-lock που κατέστρεφε routes + edge cases.

### Αποτελέσματα ✅

### 1. Attachment Field Type
**Αρχεία:** `lib/features/collections/collections_screen.dart`, `collection_detail_screen.dart`, `collection_entries_screen.dart`
- Προσθήκη `FieldType.attachment` στο enum (collections_screen.dart:69)
- `_AttachmentField` widget: file picker, image thumbnail/file icon, file name + size, delete button
- `_fieldAttachmentIds` map (`Map<String, List<int>>`) — πολλαπλά attachments ανά πεδίο
- `_loadProps`: φόρτωση attachment IDs από `ItemProperty.value` (JSON array)
- `_saveData`: αποθήκευση attachment IDs ως `jsonEncode(ids)`
- `_addAttachment`: `AttachmentService.pickAndSave` → `ref.invalidate(attachmentsProvider)`
- `_removeAttachment`: delete από service + invalidate provider
- DebugConfig schema log για επιβεβαίωση save

### 2. Auto-Lock Overhaul (Lock → Overlay)
**Πρόβλημα:** File picker → app pause → auto-lock → GoRouter redirect → route destruction → file lost

**Λύση:** Το `LockScreen` εμφανίζεται ως **overlay** πάνω από όλα αντί για GoRouter redirect.

Αλλαγές:
| Αρχείο | Αλλαγή |
|--------|--------|
| `lib/main.dart` | `MaterialApp.builder`: Stack με `child!` + `Consumer(appLockStateProvider) → LockScreen` overlay |
| `lib/core/router/app_router.dart` | Αφαίρεση lock import, `AppRoutes.lock`, redirect guard (locked→/lock), lock route |
| `lib/main.dart` | Auto-lock timer 2sec (όχι άμεσο) στο `paused` — ακύρωση στο `resumed` |
| `lib/main.dart` | `import 'dart:async'` για Timer |

### 3. File Size Limit (Configurable)
- `lib/models/app_settings.dart`: +`int maxAttachmentSizeMB = 20`
- `build_runner` — αναγέννηση `.g.dart`
- `lib/services/attachment_service.dart`: `pickAndSave(maxSizeBytes)` — `FormatException` αν υπέρβαση
- `lib/features/collections/collection_entries_screen.dart`: `_addAttachment` διαβάζει `settings.maxAttachmentSizeMB`
- `lib/features/settings/settings_screen.dart`: νέο `_MaxAttachmentSizeTile` με bottom sheet (1/5/10/20/50/100 MB, Χωρίς όριο)
- `lib/services/migration_service.dart`: `_targetVersion` 1→2, `_v1ToV2` fix AppSettings.maxAttachmentSizeMB (αν <0 ή >1000 → 20)

### Γνωστά Edge Cases (επόμενα)
1. **Dedup (ίδιο αρχείο)** — πριν save, έλεγχος αν υπάρχει Attachment με ίδιο fileName + fileSize
2. **Dedup ανά πεδίο** — `if (ids.contains(id)) return;`
3. **Type restriction** — allowed extensions
4. **Storage cleanup** — cascade delete με entry
5. **Όριο πλήθους** — max attachments ανά πεδίο
6. **Filename sanitization** — ειδικοί χαρακτήρες
7. **Multi-select** — `allowMultiple: true`

### Αρχεία που άλλαξαν
| Αρχείο | Αλλαγή |
|--------|--------|
| `lib/features/collections/collections_screen.dart` | +FieldType.attachment enum |
| `lib/features/collections/collection_detail_screen.dart` | +DebugConfig schema log |
| `lib/features/collections/collection_entries_screen.dart` | +_AttachmentField, _fieldAttachmentIds, _addAttachment/_removeAttachment, ref.invalidate, SuperNoteHelper import |
| `lib/services/attachment_service.dart` | +maxSizeBytes param |
| `lib/models/app_settings.dart` | +maxAttachmentSizeMB |
| `lib/services/migration_service.dart` | _targetVersion 1→2, _v1ToV2 fix |
| `lib/core/router/app_router.dart` | Αφαίρεση lock route + redirect |
| `lib/main.dart` | +dart:async, timer-based auto-lock, overlay LockScreen |
| `lib/features/settings/settings_screen.dart` | +_MaxAttachmentSizeTile |
| `lib/providers/providers.dart` | unchanged (lock provider already exported) |

### Backups δημιουργήθηκαν
- `collection_detail_screen.dart.bak`
- `collection_entries_screen.dart.bak`
- `attachment_service.dart.bak`
- `migration_service.dart.bak`
- `app_router.dart.bak`
- `main.dart.bak`

---

## Session 41 — 12-06-2026 (Part 2: Attachment Edge Cases #1-#3)

### Στόχος
Υλοποίηση edge cases για attachments: dedup (global & per-field) + type restrictions (allowed extensions).

### Αποτελέσματα ✅

### Edge Case #1 & #2: Dedup (ίδιο αρχείο + ίδιο ID ανά πεδίο)

**Πρόβλημα:** Χωρίς έλεγχο, το ίδιο αρχείο ανέβαινε πολλές φορές → duplicate files στο disk + duplicate DB records.

**Λύση (δύο επίπεδα):**

1. **Global dedup στο `AttachmentRepository.findDuplicate`** (`super_note_helper.dart:1323-1335`):
   - Isar query: `fileNameEqualTo + fileSizeEqualTo` (χωρίς `itemIdEqualTo`)
   - Αν βρεθεί οποιοδήποτε Attachment με ίδιο όνομα + μέγεθος → επιστρέφεται το υπάρχον
   - `pickAndSave` το επιστρέφει χωρίς copy file / νέα DB εγγραφή

2. **Per-field dedup στο `_addAttachment`** (`collection_entries_screen.dart:815-818`):
   - `if (_fieldAttachmentIds.values.any((ids) => ids.contains(attachment.id)))`
   - Αν το attachment ID υπάρχει ήδη σε οποιοδήποτε πεδίο → snackbar + return

### Edge Case #3: Type Restriction (Allowed Extensions)

**Προσέγγιση:** Per-field configuration στο schema (πλήρως συμβατό με την αρχιτεκτονική).

**Αλλαγές:**

| Αρχείο | Αλλαγή |
|--------|--------|
| `collections_screen.dart:72-143` | +`List<String> allowedExtensions` στο `FieldDef`, presets `imagesExt`/`documentsExt`, serialization σε JSON (δεν γράφεται αν empty = backward compat) |
| `collection_detail_screen.dart:224-391` | Field editor: `extensionsCtrl` + `activePresets` Set + `FilterChip` (Εικόνες/Έγγραφα) που προστίθενται/αφαιρούνται σωρευτικά στο text field. Νέο helper `_togglePresetExts` |
| `collection_entries_screen.dart:805-813` | `_addAttachment` διαβάζει `field.allowedExtensions` και τα περνάει στο `pickAndSave` |
| `attachment_service.dart:23` | `DebugConfig.db` log με extensions + maxBytes στην αρχή του `pickAndSave` |

### Schema Editor UI
- Collections → τρεις τελείες → Επεξεργασία → Πεδία → + (νέο πεδίο)
- Όταν επιλέγεις τύπο "Συνημμένο" → εμφανίζονται 2 FilterChip: **Εικόνες** και **Έγγραφα**
- Τα checkboxes λειτουργούν σωρευτικά (όχι αντικατάσταση)
- Το text field ενημερώνεται αυτόματα και είναι επεξεργάσιμο χειροκίνητα
- Κενό = όλοι οι τύποι επιτρέπονται

### Files that changed
| File | Change |
|------|--------|
| `lib/features/collections/collections_screen.dart` | +allowedExtensions, presets, toJson/fromJson |
| `lib/features/collections/collection_detail_screen.dart` | +extensionsCtrl, activePresets, FilterChip, _togglePresetExts, removed _ExtPresetChip |
| `lib/features/collections/collection_entries_screen.dart` | _addAttachment passes allowedExtensions to pickAndSave |
| `lib/services/attachment_service.dart` | +DebugConfig.db log with extensions |
| `lib/helpers/super_note_helper.dart` | findDuplicate global (no itemId filter) |

### Backups δημιουργήθηκαν
- `super_note_helper.dart.bak`
- `attachment_service.dart.bak`
- `collection_entries_screen.dart.bak`
- `collections_screen.dart.bak`
- `collection_detail_screen.dart.bak`

### Next (pending edge cases)
4. Storage cleanup / cascade delete
5. Όριο πλήθους αρχείων ανά πεδίο
6. Filename sanitization
7. Multi-select

---

## Session 42 — 12-06-2026 (Part 3: Attachment Edge Cases #4-#6 + Preview + Save)

### Στόχος
Υλοποίηση edge cases #4-6 (storage cleanup, max files limit, filename sanitization) + προσθήκη preview και save-to-disk για attachments.

### 1. Storage cleanup / Cascade delete (Edge Case #4) ✅

**Πρόβλημα:** Όταν διαγράφεται μια εγγραφή collection, τα attachments της έμεναν ορφανά (DB records + φυσικά αρχεία).

**Λύση** (`super_note_helper.dart`):
- `hardDelete` cascade: DB records + disk files (φυσική διαγραφή)
- `softDelete` untouched (restore-safe)

### 2. maxFiles επιλογή (Edge Case #5) ✅

**Αλλαγές:**

| Αρχείο | Αλλαγή |
|--------|--------|
| `collections_screen.dart` | `+int maxFiles = 0` στο `FieldDef`, serialization |
| `collection_detail_screen.dart` | +TextField + hint "1 έως 10", `(int.tryParse(...) ?? 1).clamp(1, 10)` |
| `collection_entries_screen.dart` | `_addAttachment`: `if (field.maxFiles > 0) { if (current >= field.maxFiles) return; }` |

**Διευκρίνιση:** 0 = unlimited → μετά από εντολή χρήστη, άλλαξε σε **strict 1-10** (`clamp(1, 10)`, χωρίς 0=unlimited).

### 3. Label fix ✅
- "Επιτρεπόμενες επεκτάσεις" → "Επιτρεπόμενοι τύποι αρχείων" (`collection_detail_screen.dart`)

### 4. Filename sanitization (Edge Case #6) ✅

**Ανάλυση:** Απαγορευμένοι χαρακτήρες Windows (`\ / : * ? " < > |`), control characters (0x00-0x1F), trailing dots/spaces, reserved names (CON, PRN, AUX, NUL, COM1-9, LPT1-9).

**Υλοποίηση:**

| Αρχείο | Αλλαγή |
|--------|--------|
| `string_utils.dart` | Νέα `AppStringUtils.sanitizeFileName()` — reserved chars→`_`, trailing dots trim, reserved names→`reserved_` prefix, fallback `unnamed_file` |
| `attachment_service.dart` | `pickAndSave`: sanitize `file.name` → `cleanName`, χρήση σε `findDuplicate` + create. `saveFile`: sanitize `p.basename(sourcePath)` |

**Backward compatibility:** old records untouched. `findDuplicate` συγκρίνει πλέον sanitized names (νέα records match μεταξύ τους).

**TestCase (από logs):** `CON_report.docx` → ΔΕΝ χρειάστηκε sanitization (stem `CON_report`, όχι ακριβώς `CON`).

### 5. Attachment Preview + Save to Disk ✅

**Νέο package:** `open_filex: ^4.5.0` (`pubspec.yaml`)

**Νέα συμπεριφορά κάρτας** (`collection_entries_screen.dart`):
...
| Ενέργεια | Συμπεριφορά |
|----------|-------------|
| Tap στην κάρτα | Image → fullscreen `InteractiveViewer` dialog. Άλλο → `OpenFilex.open()` με system app |
| 📥 Save | `FilePicker.platform.saveFile()` (bytes) — ο χρήστης επιλέγει πού |
| ✕ Delete | `AttachmentService.delete()` (DB + disk), εκτός κάρτας (αποφυγή λάθος πατημάτων) |

**Layout restructure:** X button μεταφέρθηκε έξω από την κάρτα. Save icon μέσα στην κάρτα (δεξιά).

**New callbacks chain:** `_CollectionEntryDetailScreenState` → `_openAttachment()` + `_saveAttachment()` → `_FieldInput` (onOpenAttachment, onSaveAttachment) → `_AttachmentField` (onOpen, onSave)

### DebugConfig που προστέθηκαν

| Σημείο | Log |
|--------|-----|
| `pickAndSave` sanitize | `pickAndSave sanitize: "..." → "..."` |
| `saveFile` sanitize | `saveFile sanitize: "..." → "..."` |
| `saveFile` store | `saveFile: stored fileName="..." path="..." size=...` |
| `_addAttachment` limit | `_addAttachment maxFiles check: field="..." current=N max=N` |
| `collection_detail_screen` | `maxFiles: "N" clamped → N` |
| `_openAttachment` | `_openAttachment id=N type="..."` |
| `_saveAttachment` | `_saveAttachment id=N file="..."` → `saved to "..."` or `cancelled` |

### Δοκιμή (από emulator logs)
- `_openAttachment id=6 type=image/jpeg` → fullscreen preview ✅
- `_openAttachment id=11 type=image/jpeg` → fullscreen preview ✅
- `_saveAttachment id=11 file="IMG_20260406_124146.jpg"` → **saved to `/storage/emulated/0/Download/`** ✅

### Backups δημιουργήθηκαν
- `backups/string_utils.dart.backup.20260612`
- `backups/attachment_service.dart.backup.20260612`
- `backups/collection_entries_screen.dart.backup.20260612`

### Αρχεία που άλλαξαν

| Αρχείο | Αλλαγή |
|--------|--------|
| `pubspec.yaml` | +open_filex ^4.5.0 |
| `lib/core/utils/string_utils.dart` | +sanitizeFileName() |
| `lib/services/attachment_service.dart` | sanitizeFileName σε 2 σημεία, +Debugs |
| `lib/features/collections/collection_detail_screen.dart` | maxFiles 1-10, label fix, hints, debug |
| `lib/features/collections/collection_entries_screen.dart` | _openAttachment, _saveAttachment, layout change, new callbacks chain, debugs |
| `lib/features/collections/collections_screen.dart` | maxFiles field (unchanged from Session 41) |

### Εκκρεμότητες
- [ ] Multi-select (`allowMultiple: true`) — **Cancelled** (user: αφού μπορεί να ανεβάσει 10 αρχεία σε όσα πεδία θέλει, δεν χρειάζεται)
- [ ] Share (θα ενταχθεί μελλοντικά στο share του Item)
- ~~[ ] Έλεγχος auto-lock timeout setting~~ ✅ Διορθώθηκε — Session 43

---

## Session 43 — 12-06-2026 (Auto-lock timeout + Collection entry share)

### 1. Auto-lock timeout fix
Η ρύθμιση `appLockTimeoutSeconds` αγνοούνταν — χρησιμοποιούσε hardcoded 2s.

`main.dart:264`: `Duration(seconds: 2)` → `Duration(seconds: settings.appLockTimeoutSeconds)`

### 2. Collection entry share (πεδία + attachments)
Το share collection entry έδειχνε μόνο τίτλο (default case). Τώρα:
- Φορτώνει schema από το parent collection
- Μορφοποιεί όλα τα πεδία (text, number, date, toggle, select, url, bulletList, numberedList, attachment)
- Συμπεριλαμβάνει τα attachment files ως `ShareParams(files: [...])`

Αποφεύχθηκε circular dependency (JSON parsing αντί για import FieldDef/FieldType).

### Verify
- `flutter analyze` — **No issues found**
- Backups: `main.dart.backup.20260612`, `share_service.dart.backup.20260612`

### Αρχεία που άλλαξαν
| Αρχείο | Αλλαγή |
|--------|--------|
| `main.dart:264` | hardcoded 2s → `settings.appLockTimeoutSeconds` |
| `share_service.dart` | `shareItem` branching + `_buildKnowledgeShare` + `_formatCollectionEntry` + DebugConfig logs |

### DebugConfig logs (share)
`📤 _buildKnowledgeShare itemId=... collectionId=...` / `📤 Schema loaded=...` / `📤 Fields parsed count=...` / `📤 Attachment field "..." ids=...` / `📤  + file id=... "..." path="..."` / `📤  ⚠️ attachment parse error` / `📤 Share itemId=... type=knowledge files=...`
